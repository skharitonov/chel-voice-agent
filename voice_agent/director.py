...
_A_DIRECTIVES = {
    StageA.GATEKEEPER_INTRO.value:
        "ФАЗА СЕКРЕТАРЬ. Открытие ОДНОЙ короткой фразой: имя + ЧЗЭ + просьба "
        "соединить с энергетиком по подстанциям с истекающим сроком. НЕ перечисляй "
        "детали закона/ГОСТ/типы — это для ЛПР. Без вопросов про парк, без продукта.",
    ...
    StageA.GATEKEEPER_EXTRACT.value:
        "ФАЗА СЕКРЕТАРЬ, ТУПИК. Соединения нет — извлеки хотя бы одно: ФИО+должность "
        "ЛПР, личную почту, добавочный ИЛИ дату/время перезвона. Ни одна ветка не "
        "выходит пустой (Правило №1).",
...
class DialogueDirector:
    ...
    def _directive_a(self) -> str:
        ...
        if s.stage == StageA.WRAP.value:
            ...
            if s.refused:
                return "Заверши: извинись за беспокойство и попрощайся."
            ...
            if not self._has_extracted_data():
                return ("ТУПИК перед завершением (Правило №1): попроси сейчас хотя бы "
                        ...
                        "перезвона; затем вежливо попрощайся.")
            return "Заверши разговор вежливо, подтверди договорённость."
