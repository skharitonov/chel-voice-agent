    ...
    def _directive_a(self) -> str:
        ...
        if s.stage == StageA.WRAP.value:
            ...
            return ("Заверши разговор вежливо, подтверди договорённость, попрощайся "
                    "словами «До свидания».")
        if s.stage == StageA.GATEKEEPER_EXTRACT.value:
            # Разводим по ходам: сначала ДОБРАТЬ ФИО, потом отдельно ПЕРЕКЛЮЧИТЬ.
            base = ("ФАЗА СЕКРЕТАРЬ, ТУПИК. ВАЖНО: имя, которое назвал секретарь в "
                    "ответ на «как зовут энергетика» — это имя ЛПР (энергетика), НЕ "
                    "секретаря. НЕ обращайся к секретарю этим именем. ")
            if s.extract_rounds == 0:
                return base + ("Узнай ФИО энергетика ПОЛНОСТЬЮ — имя, отчество, "
                               "фамилию. В ЭТОЙ реплике НЕ проси переключить, только "
                               "уточни недостающие части ФИО одним коротким вопросом.")
            return base + ("ФИО энергетика уже узнал. Теперь, если разговор идёт ровно, "
                           "попроси ПЕРЕКЛЮЧИТЬ на него по имени-отчеству. Если "
                           "переключить нельзя — возьми личную почту/добавочный или "
                           "назначь перезвон.")
